...
export class HomeComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.title.setTitle('La Flor de Maria');
    this.meta.updateTag({ name: 'descripcion', content: 'Tienda virtual para floreria' });
  ...
  addToCart(id: string): void {
    ...
    this.translate.getTranslations$()
      .pipe(map(translations => translations
        ? {message: translations['ADDED_TO_CART'] || 'Agredado a la cesta', action: translations['TO_CART'] || 'Ir a la cesta'}
        : {message: 'Agredado a la cesta', action: 'Ir a la cesta'}
        ),take(1))
